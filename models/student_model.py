...
class StudentModel(nn.Module):
    ...
    """
    def __init__(self, image_size, in_channel=3, num_blocks=None, channels=None, patch_size=(2, 2), num_classes=8, use_distillation=True, heads=8, expansion=4, dim_head=32, dropout=0., emb_dropout=0., **kwargs):
        super().__init__()
        ...
        feature_resolution = (ih // 2, iw // 2)  # 对于 28x28 的输入，经过 2 个 stage 后，特征图大小为 14x14 
...
def student_model(**kwargs):
    num_blocks = [2, 2, 3, 3]
    channels = [64, 96, 192, 256]
    return StudentModel((28, 28), in_channel=3, num_blocks=num_blocks, channels=channels, patch_size=(2, 2), use_distillation=False, **kwargs)

@register_model
def HDKD(**kwargs):
    num_blocks = [2, 2, 3, 3]
    channels = [64, 96, 192, 256]
    return StudentModel((28, 28), in_channel=3, num_blocks=num_blocks, channels=channels, patch_size=(2, 2), use_distillation=True, **kwargs)