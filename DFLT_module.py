...
class DFLT(nn.Module):
    ...
    def __init__(self, image_size, patch_size, dim, depth, heads, expansion, channels, use_distillation=True, dim_head=32, dropout=0., emb_dropout=0.):
        ...
        # 根据输入张量的形状动态调整 pos_embedding 的形状
    ...
    def forward(self, img):
        ...
        # 动态调整 pos_embedding 的形状
        if x.shape[1] != self.pos_embedding.shape[1]:
            print(f"Adjusting pos_embedding shape from {self.pos_embedding.shape} to {x.shape}")  # 调试输出
            self.pos_embedding = nn.Parameter(torch.randn(1, x.shape[1], self.dim).to(x.device))  # 确保在相同设备上
